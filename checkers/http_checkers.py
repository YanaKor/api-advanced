...
@contextmanager
def check_status_code_http(expected_status_code, expected_message):
    try:
        yield
        ...
        if expected_message:
            raise AssertionError(f'Должно быть получено сообщение "{expected_message}", но запрос прощел успешно')
